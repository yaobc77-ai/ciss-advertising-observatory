...
**Research preview: the real native corpus is connected and the application runs locally.** Version 0.2.5 contains 275 records, with 263 countable records, 226 eligible retrieval bodies and 558 source-located chunks. Data version: `5114ebc1cf9afe59cdaa715e3ea45166`. One article includes reviewed partial continuation from PDF-265; its old text, evidence and label basis remain preserved. The dashboard, filtered CSV export, free keyword search and paid evidence answers are implemented. See the [Dashboard consistency fix](reports/dashboard_consistency_v0_2_5.md), [current backup/restore check](reports/backup_restore_v0_2_4.md), [0.2.4 quote context and reproduction report](reports/quote_context_v0_2_4.md), [recovery report](reports/pdf265_body_recovery.md), [publication checks](outputs/pdf265_publication_validation_20260916.json), [handoff](docs/handoff.md) and [acceptance status](docs/acceptance_status.md).
...
Version **0.2.5 passed 253 engineering tests (22.07 seconds)**. Within each Dashboard refresh, chart statistics now use the same records as the table; unknown dates remain in summary counts and are excluded from the monthly timeline. The installed wheel and restarted local app were checked. Retrieval, generation, data and evaluation cases did not change, and no paid evaluation was repeated for this UI fix. The following 0.2.4 generation and import results retain their original version.
