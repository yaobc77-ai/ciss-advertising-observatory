...
| M5 RAG | 关键词/向量检索、付费回答、原文区间引用、预算；Lingua 本地错语检查及保留费用的失败处理 | 不确定语言、归因/单位对象/语义充分性与跨库验证 |
| M6 评价 | 20 开发题＋20 验收草案；当前版本 15/17 段 gold 前置校验有效；0.2.1 工程全套 164 通过 | 草案审题、冻结与独立人工语义验收；7 个社交/跨库题尚未运行 |
| M7 部署 | 本机 PostgreSQL+pgvector、生产服务、预算、限额、备份恢复脚本；本机健康通过 | 固定域名/Tunnel 及另一网络的公众验收 |
| M8 交接 | 0.2.1 源码、依赖锁、配置、wheel、文档及历史预览材料 | 指定 GitHub、完整双数据集最终展示、客户与交接验收 |
...
最新付费诊断 `612863…` 绑定当前 `d85…`：记录命中 8/8、支持片段 13/13、引用定位 16/16、拒答 3/3；语言 7 match、1 inconclusive，`overall_pass=null`。旧 `532453… / c5ad…` 的三条错语结果保留并用于离线回归，未被覆盖。语言修复没有解决所有归因和单位对象问题，不能称已完成语义验收。社交数据为空不计为通过。
