...
当前主库版本为 `d85a98002e4493f0376c260ad82253ee`，有275条收录、263条可计数、226条可检索及554个当前片段，媒体分组为8个。它采用两份绑定源路径／文件哈希／逻辑行号／正文哈希的 AI 工程配置：新增候选7纳入、4排除、1待核；20处导航边界按有序原文区间采纳。CLI 全快照要求两份配置均存在并通过校验。配置变动参与记录版本，旧运行不能自动当成本版本的成绩。
